...
private:
    // 用于文本日志的滚动文件管理器（替换原来的 log_file/current_log）
    RollingFileManager text_mgr{
        std::filesystem::path("./logs/text"),   // 基础目录（可先写死，后面第4天换成JSON）
        "log_%Y%m%d_%H%M%S_%03d.txt",           // 命名模式
        1 * 1024 * 1024,                       // 单文件上限 10MB
        std::chrono::minutes(1),               // 或 60 分钟轮转
        8,                                      // 仅保留最近 8 个（含 .gz）
        true                                    // 轮转后压缩旧文件
    };
    ...
    std::ofstream  binary_file, bag_file;
    ...
    void logSync(LogLevel level, const std::string& message,
        ...
        oss << getCurrentTime() << " " << logLevelToString(level)
            << " " << file << ":" << line << " " << function
            << " - " << message;
        const std::string lineStr = oss.str(); // FIX: 定义 lineStr
    ...
    void logBinaryAsync(const void* data, size_t size, const std::string& tag) {
     ...
     // 异步线程消费的二进制落盘函数
    void writeBinaryEntry(const BinaryEntry& e) {
        std::lock_guard<std::mutex> lock(binary_mtx);
        if (!binary_file.is_open()) return;
        ...
        while (!stop) {
            {
                ...
                cv.wait_for(lock, std::chrono::milliseconds(500), [this] {
                    return !front_buffer.empty() || !front_message_buffer.empty() 
                    || !binary_front.empty()   // FIX: 等待二进制缓冲
                    || stop;
                });
                front_buffer.swap(back_buffer);
                front_message_buffer.swap(back_message_buffer);
                binary_front.swap(binary_back);   // FIX: 交换二进制缓冲
            }
            ...
            if (!binary_back.empty()) {           // FIX: 消费二进制
                for (const auto& e : binary_back) writeBinaryEntry(e);
                binary_back.clear();
            }
        }

        // FIX:flush 残留
        for (const auto& e : front_buffer) writeLog(e);
        front_buffer.clear();

        for (const auto& e : front_message_buffer) writeMessage(e);
        front_message_buffer.clear();

        for (const auto& e : binary_front) writeBinaryEntry(e); // FIX: flush 二进制残留
        binary_front.clear();
    }
    ...
    void writeLog(const LogEntry& entry) {
        ...
        if (os.good()) {
            os << log_msg << std::endl;
            // 异步路径通常可以不 flush，每隔若干条或定时 flush 一次；这里为了简单保持 flush
            os.flush();
        }
...
int main() {
    ...
    LOG_INFO("sync: hello");
    LOG_WARNING("sync: a warning");
    LOG_ERROR("sync: an error");

    // ---- 2) 异步模式：切换 ----
    std::cout << "== Switch to async ==\n";
    logger.setAsyncMode(true);

    // ---- 3) 连续写大日志，触发 10MB 轮转（旧文件应被 .gz 压缩）----
    // 1KB 级 payload，循环若干次逼近/超过 10MB
    const std::string payload(1024, 'X');   // ~1KB
    const int ITERATIONS = 15000;           // 约 15MB（含格式开销）
    for (int i = 0; i < ITERATIONS; ++i) {
        LOG_INFO(std::string("async bulk #") + std::to_string(i) + " " + payload);
        // 适当让出时间，避免占满 CPU（可选）
    ...
    std::cout << "Done. 请到 ./logs/text/ 下查看：应看到旧的 .txt 被压缩为 .gz，新建了一个最新 .txt。\n";
    std::cout << "如未触发轮转：可把 ITERATIONS 调大，或把 RollingFileManager 的 maxBytes 临时调小。\n";
    return 0;
}
