...
void LoggerCore::processLogs() {
    while (!stop_) {
        {
            ...
            text_front_.swap(text_back_);
            binary_front_.swap(binary_back_);
            message_front_.swap(message_back_);
        }
        
        // 处理文本日志
        if (!text_back_.empty()) {
            for (const auto& e : text_back_) {
                logSync(e);
            }
            text_back_.clear();
        }
        ...
        // 处理二进制日志
        if (!binary_back_.empty()) {
            for (const auto& e : binary_back_) {
                logBinarySync(e);
            }
            binary_back_.clear();
        }
        ...
        // 处理消息记录
        if (!message_back_.empty()) {
            for (const auto& e : message_back_) {
                recordMessageSync(e);
            }
            message_back_.clear();
        }
    ...
    // 刷新残留数据
    for (const auto& e : text_front_) logSync(e);
    for (const auto& e : binary_front_) logBinarySync(e);
    for (const auto& e : message_front_) recordMessageSync(e);
}
